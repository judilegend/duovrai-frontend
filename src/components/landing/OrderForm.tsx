...
function FormField({
  ...
  return (
    <div className="space-y-1.5">
      <label className="block text-[12px] font-semibold text-[#1A5C52] uppercase tracking-widest">
        {label}
        {required && <span className="text-[#B8962E] ml-0.5">*</span>}
      </label>
      <input
        type={type}
        ...
        onBlur={onBlur}
        className={`w-full px-4 py-2.5 rounded-[8px] border text-[14px] text-[#1A1A1A] bg-white transition-all focus:outline-none focus:ring-2 ${
          hasError
            ? "border-red-300 focus:ring-red-200"
            : "border-[#E8F2F0] focus:ring-[#1A5C52]/20 focus:border-[#1A5C52]/60"
        }`}
      />
      {hasError && (
    ...
    <section id="formulaire" className="py-16 sm:py-24 bg-white">
      <div className="container mx-auto px-4 sm:px-6">
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center max-w-5xl mx-auto">
          ...
          >
            <div>
              <p className="duovrai-label text-[12px] mb-3">Découvrez votre alchimie</p>
              <h2 className="text-3xl sm:text-4xl mb-4 leading-tight">
                Prêt à explorer votre destinée ensemble ?
              </h2>
              <p className="text-base text-[#555555] leading-relaxed sm:text-lg">
            ...
            <div className="space-y-3 mt-6">
              {[
                ...
                <div key={i} className="stagger-item flex items-start gap-3 animate-slide-up">
                  <svg
                  ...
                  <span className="text-sm sm:text-base text-[#555555]">{item}</span>
                </div>
              ))}
            </div>
          </motion.div>
          ...
          >
            <div className="mb-6 text-center">
              ...
              <h3 className="mt-3 text-2xl sm:text-3xl">Démarrez maintenant</h3>
            ...
            <form
              ...
              className="rounded-[28px] border border-[#E8F2F0] bg-white/95 p-5 shadow-[0_18px_50px_-30px_rgba(26,92,82,0.45)] backdrop-blur sm:p-7 space-y-5"
              noValidate
            >
              {/* Personne 1 */}
              ...
              <button
                type="submit"
                className="duovrai-btn group mt-2 h-12 w-full bg-[#1A5C52] text-base font-semibold text-white shadow-lg shadow-[#1A5C52]/10 transition-all hover:-translate-y-0.5 hover:bg-[#173F39] sm:h-14 rounded-[6px] flex items-center justify-center gap-2"
              >
                Procéder au paiement
                <ArrowRight className="ml-2 h-4 w-4 transition-transform group-hover:translate-x-1" />
              </button>

              <div className="flex items-center justify-center gap-2 rounded-2xl border border-[#E8F2F0] bg-[#F9FCFB] px-3 py-2 text-xs text-[#555555] sm:text-sm">
